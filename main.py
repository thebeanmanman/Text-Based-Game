...
def shopEncounter():
    if player.gold > 0:
        text(f"Shopkeeper: {col.npc('Hello there young traveller, take a look at my fine wares.')}")
        shop = Shop(sellWeapons=True,name="Shopkeeper's Store")
        shop.printItems()
        shop.enterShop(player)
        enterDungeon()
    else:
        text(f"Shopkeeper: {col.npc('Hello there young traveller, please come back to my store when your pockets are full of gold.')}")
        enterDungeon()
